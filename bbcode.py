...
def format_rule_location(rule_location):
    if isinstance(rule_location, OpnSenseRuleAlias):
        rule_location = format_rule_alias(rule_location.data)
    return str(rule_location)

def format_bbcode_cell(cell):
    if cell is None or (isinstance(cell, OpnSenseNode) and cell.data is None):
        cell = ''
    elif cell is True or (isinstance(cell, OpnSenseNode) and cell.data is True):
        cell = 'x'
    elif isinstance(cell, OpnSenseRuleAlias):
        cell = format_rule_alias(cell.data)
    elif isinstance(cell, OpnSenseRuleInterface):
        cell = format_rule_interface(cell.data)
    elif isinstance(cell, OpnSenseRuleLocation):
        if hasattr(cell, 'not'):
            data = '**!** '
        else:
            data = ''
...
def output_bbcode(doc, stream):
    ...
    if hasattr_r(doc.opnsense, 'OPNsense.Firewall.Alias.aliases.alias'):
        ...
        output_bbcode_table(stream, ('Name', 'Type', 'Content', 'Description', 'Detail'), aliases)
        stream.write("\n")
